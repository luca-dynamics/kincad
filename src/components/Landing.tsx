  ...
  return (
    <div className="flex h-full flex-col items-center justify-center px-6">
      <div className="w-full max-w-2xl">
        ...
        <h1 className="mb-2 text-center text-3xl font-semibold tracking-tight text-fg">
        ...
        <p className="mb-8 text-center text-sm text-muted">
          ...
          question, or open one from the sidebar. Every number comes from the deterministic
          solver.
        </p>
        ...
        <div className="mt-5 flex flex-wrap justify-center gap-2">
          {PROMPTS.map((p) => (
            <button
              key={p}
              onClick={() => onSend(p)}
              className="rounded-full px-3 py-1.5 text-[12px] text-muted ring-1 ring-line transition-colors hover:text-accent hover:ring-accent/50"
            >
              {p}
            </button>
          ))}
