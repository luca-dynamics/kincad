...
export default function TopBar({
  ...
  const exportBtn = isCad ? (
    <button onClick={onExportSTL} title="Export STL" className="flex items-center gap-1.5 rounded-md bg-accent px-2.5 py-1.5 text-xs font-medium text-accent-fg transition-opacity hover:opacity-90">
      <Download className="h-4 w-4" />
      <span className="hidden sm:inline">STL</span>
    </button>
  ) : (
    <button onClick={onExportPDF} title="Download report PDF" className="flex items-center gap-1.5 rounded-md bg-accent px-2.5 py-1.5 text-xs font-medium text-accent-fg transition-opacity hover:opacity-90">
      <Download className="h-4 w-4" />
  ...
  return (
    <div className="flex-shrink-0 border-b border-line bg-panel">
      {/* ── Row 1: tabs + transport icons + export ── */}
      <div className="flex h-11 items-center gap-2 overflow-hidden px-3">
        {viewTabs}
        ...
        {/* θ₂ slider — hidden on mobile, lives in row 2 */}
        ...
        {/* right-hand icon toggles — hide on mobile */}
        ...
        {/* spacer pushes export button to the right on mobile */}
        <div className="flex-1 sm:hidden" />

        {exportBtn}
      </div>

      {/* ── Row 2 (mobile only): full-width θ₂ slider ── */}
      <div className="flex items-center gap-2 px-3 pb-2 sm:hidden">
        ...
        />
        <span className="num w-9 flex-shrink-0 text-right text-[11px] text-muted">{deg.toFixed(0)}°</span>
      </div>
    </div>
  );
}
